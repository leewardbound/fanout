...
def test_actor_factory(local_fanout):
    ...
    assert models.Actor.objects.count() == len(
        local_fanout.other_actors) + 2, "there should be N+1 actors"
...
def test_local_fanout_fixture(local_fanout):
    ...
    assert 'https://localhost/' in actor.id
    assert models.Actor.objects.count() == len(
        local_fanout.other_actors) + 1, "there should be N actors (plus 1 service account for the domain)"
