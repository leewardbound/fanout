...
class Customer(AuditableMixin, TimestampMixin):
    ...
    def collect_customer(cls, email, user=None, ip=None, datapoints: Mapping[str, str] = None):
        ...
        if user:
            try:
                ...
                existing_by_user.create_audit_log(AuditLogType.CUSTOMER_UPDATED, 'Found customer by user session', performing_user=user,
        ...
        try:
            ...
            existing_by_email.create_audit_log(AuditLogType.CUSTOMER_UPDATED, 'Found customer by email address', performing_user=user,
                                              extra_data={"ip": ip, **datapoints})
        ...
        customer.create_audit_log(AuditLogType.CUSTOMER_CREATED, performing_user=user,
                                           extra_data={"ip": ip, **datapoints})
        return customer
    ...
    def subscribe_to_actor(self, actor_id):
        ...
        sub.create_audit_log(audit_log_type, performing_user=self.user,
                             extra_data=self.get_all_data())
...
class Subscription(AuditableMixin, TimestampMixin):
    ...
    type = models.CharField(max_length=32, default=FollowOptions.FOLLOW,
                            choices=FollowOptions.choices)
